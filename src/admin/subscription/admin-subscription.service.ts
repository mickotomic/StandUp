...
export class AdminSubscriptionService {
  ...
  async getSubscriptionsList(query: PaginateQuery): Promise<Paginated<Subscription>> {
    ...
    const qb = this.subscriptionRepository.createQueryBuilder('subscriptions')
    .leftJoinAndSelect('subscriptions.workspace' , 'workspace')
    .leftJoinAndSelect('workspace.owner', 'owner');
    return await paginate<Subscription>(query, qb, paginateConfig);
  }
  async updateSubscription(subscriptionId: number, updateAdminSubscriptionDto: UpdateAdminSubscriptionDto){
